...
public class JoinValidator implements Validator<RequestJoin>,
    ...
    public void check(RequestJoin form) {
           ...
           //필수 항목 검증 - 이메일, 비밀번호, 비밀번호 확인, 회원명, 약관동의
            ...
            //비밀번호, 비밀번호 확인 일치 여부
            checkTrue(password.equals(confirmPassword),
            ...
            //비밀번호 자리수 체크
            checkTrue(password.length() >= 8,
                    new BadRequestException("비밀번호는 8자리 이상 입력하세요."));

            //이미 가입된 회원인지 체크
            checkTrue(mapper.exist(email)==0L,
                    new DuplicatedMemberException());
    }
}
